...
int main( int i_argc, char *i_argv[] ) {
  ...
  auto l_printUsage = []() {
    ...
    std::cerr << "  ./build/tsunami_lab NX NY X_LOWER X_UPPER Y_LOWER Y_UPPER K SOLVER_MODE END_TIME [USE_CUDA] PROPAGATION [PROP_PARAMS] SETUP [SETUP_PARAMS]" << std::endl;
    std::cerr << "  ./build/tsunami_lab <RES>m  X_LOWER X_UPPER Y_LOWER Y_UPPER K SOLVER_MODE END_TIME [USE_CUDA] PROPAGATION [PROP_PARAMS] SETUP [SETUP_PARAMS]" << std::endl;
    std::cerr << "" << std::endl;
  ...
  while( l_simTime < l_endTime ){
    if( l_timeStep % 25 == 0 ) {
      if (l_wavePropGpu != nullptr) {
        l_wavePropGpu->copyToHost( l_waveProp->getHeight(),
                                 l_waveProp->getMomentumX(),
                                 l_waveProp->getMomentumY() );
      } 
