...
same netCDF fallback module that made ``which nc-config`` work during
compilation.
...
the solver now prints ``time stepping seconds``, ``time steps``, and ``time per
cell and iteration`` at the end of each run.
...
command printed by ``scons --debug=presub``; it should use ``g++``, not ``gcc``,
and the link command should contain ``-lstdc++fs`` after ``-lnetcdf``:
...
Stop it and request a Slurm allocation with ``salloc`` or submit through
``sbatch``. Use ``hostname`` to check where the shell is running.